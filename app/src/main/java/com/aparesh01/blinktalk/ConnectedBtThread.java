package com.aparesh01.blinktalk;

import android.bluetooth.BluetoothSocket;
import android.os.Handler;
import android.os.Message;
import android.util.Log;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.io.OutputStream;

public class ConnectedBtThread extends Thread {

    private final BluetoothSocket mmSocket;
    private final InputStream mmInStream;
    private final OutputStream mmOutStream;
    public static final int RESPONSE_MESSAGE = 10;
    Handler uih;

    public ConnectedBtThread(BluetoothSocket socket,Handler uih){
        mmSocket = socket;
        this.uih = uih;
        InputStream tmpIn = null;
        OutputStream tmpOut = null;
        try{
            tmpIn = mmSocket.getInputStream();
            tmpOut = mmSocket.getOutputStream();
        }catch(IOException e){
            Log.e("[Thread : CBT ]","Err : " + e.getMessage());
        }
        mmInStream = tmpIn;
        mmOutStream = tmpOut;
        try{
            mmOutStream.flush();
        }catch(IOException e){
            Log.e("[Thread : CBT]","Err : " + e.getMessage());
            return;
        }
        Log.i("[THREAD-CT]","IO's obtained");
    }

    @Override
    public void run() {
        super.run();
        BufferedReader br = new BufferedReader(new InputStreamReader(mmInStream));
        while(true){
            try{
                String str = br.readLine();
                Message msg = new Message();
                msg.what = RESPONSE_MESSAGE;
                msg.obj = str;
                uih.sendMessage(msg);
            }catch(IOException e){break;}
        }
        Log.i("[THREAD-CT]","While loop ended");
    }

    public void write(byte[] bytes){
        try{
            Log.i("[THREAD-CT]", "Writting bytes");
            mmOutStream.write(bytes);
        }catch(IOException e){
            Log.e("[Thread : CBT]", "Error : in sending msg" + e.getMessage());
        }
    }
    public void cancel(){
        try{
            mmSocket.close();
        }catch(IOException e){
            Log.e("[Thread : CBT]", "Error : in closing socket" + e.getMessage());
        }
    }
}
